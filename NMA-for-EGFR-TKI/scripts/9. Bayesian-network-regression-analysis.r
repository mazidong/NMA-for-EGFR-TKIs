# file name: 9. Bayesian-network-regression-analysis.r
...
main_output_dir <- "/your_path_to_project/NMA-for-EGFR-TKI/outcomes/Regression/" # Change to your main output directory
...
data_path = paste0(file_path,'Bayesian regression analysis-primary.xlsx')
file.path = paste0(file_path, 'Bayesian regression analysis-continuity data.xlsx')

sheet_name = excel_sheets(data_path)
sheet_name

data <- read_excel(file.path, sheet = 'Sheet1')
colnames(data)
data <- data %>%
  mutate(
...
saveWorkbook(wb, "Merged_bayesian_network_regression-data.xlsx", overwrite = TRUE)
...
data_path = paste0(main_output_dir,'Merged_bayesian_network_regression-data.xlsx')

# Binary Bayesian network meta-regression analyses
binary_output_dir <- paste0(main_output_dir, 'binary/')
if (!dir.exists(binary_output_dir)) dir.create(binary_output_dir)
setwd(binary_output_dir)
output_file = paste0(binary_output_dir, 'Bayesian network meta-regression analyses-Binary.xlsx')
for (group in Binary_list){
      for (j in outcomes){
            ...
            for(i in unique(networkreg$studies[,group])){
                  ...
                  cumrank.prob <- apply(t(ranks), 2, cumsum)
                  sucra <- round(colMeans(cumrank.prob[-nrow(cumrank.prob),]),4)
                  write.csv(sucra,paste0(group, "_", j, "_", i, ".csv"))
                  print(sucra)
...
continous_output_dir <- paste0(main_output_dir, 'continous/')
if (!dir.exists(continous_output_dir)) dir.create(continous_output_dir)
setwd(continous_output_dir)
output_file = paste0(continous_output_dir, 'Bayesian network meta-regression analyses-Continous.xlsx')
...
line_labels <- c("subsequent-line", "front-line")#0 Second_Line, 1 First_Line
...
saveWorkbook(wb, "primary outcomes-sucra-regression-binary.xlsx", overwrite = TRUE)
...
setwd(main_output_dir)
file_path <- "/your_path_to_project/NMA-for-EGFR-TKI/input/" # Change to your data folder path
file <- paste0(file_path, "primary outcomes-sucra-regression-binary.xlsx")# Use a version standardized for the therapy name for drawing
...
for (sheet in sheet_name){
  ...
  file_name = paste0('primary outcomes-sucra-regression-binary-heatmap_', sheet, ".pdf")
  pdf(file_name, width = 7.5)
  plot(p)
  dev.off()
  file_name1 = paste0('primary outcomes-sucra-regression-binary-ggpairs_', sheet, ".pdf")
  p1 <- ggpairs(
  data = data,
