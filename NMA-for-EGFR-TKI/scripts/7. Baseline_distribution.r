# file name: 7. Baseline_distribution.r
...
main_output_dir <- here("outcomes", "Distribution")

if (!dir.exists(main_output_dir)) dir.create(main_output_dir)

# The path to the data folder

data <- read_excel(here("input", "various distributed data.xlsx"))
colnames(data)
...
p <- ggplot(data1,aes(x=Author,y= Median_Age))+
  ...
  geom_hline(yintercept = 65, color = "black", linetype = "dashed", linewidth = 0.7) +
  theme(
...
pdf(file.path(main_output_dir, "all-study-median-mean-age.pdf"), width = 30, height = 10)
print(p1)
dev.off()
...
p <- ggplot(data1,aes(x=Author,y=median_follow_up))+
  geom_point(size=6,position = position_dodge(width = 0.5), aes(colour = factor(Group)))+  
  theme_bw() + 
  facet_grid(. ~ Group, scales = "free_x", space = "free_x")+
  geom_hline(yintercept = 24, color = "black", linetype = "dashed", linewidth = 0.7) +
  theme(
       axis.title.y = element_text(size = 18),
    axis.text.x = element_text(angle = 90, hjust = 1),  
    strip.text.x = element_text(angle = 90,size = 15, face = "bold"), 
...
pdf(file.path(main_output_dir, "all-study-Median follow-up.pdf"), width = 30, height = 10)
print(p1)
dev.off()
...
p <- ggplot(data1,aes(x=Author,y=Female))+
  geom_point(size=6,position = position_dodge(width = 0.5), aes(colour = factor(Group)))+  
  theme_bw() + 
  facet_grid(. ~ Group, scales = "free_x", space = "free_x")+
  geom_hline(yintercept = 50, color = "black", linetype = "dashed", linewidth = 0.7) +
  theme(
       axis.title.y = element_text(size = 18),
    axis.text.x = element_text(angle = 90, hjust = 1), 
    strip.text.x = element_text(angle = 90,size = 15, face = "bold"),  
...
pdf(file.path(main_output_dir, "all-study-female.pdf"), width = 30, height = 10)
print(p1)
dev.off()
...
p <- ggplot(data1,aes(x=Author,y=Asian))+
  geom_point(size=6,position = position_dodge(width = 0.5), aes(colour = factor(Group)))+  
  theme_bw() +
  facet_grid(. ~ Group, scales = "free_x", space = "free_x")+
  geom_hline(yintercept = 50, color = "black", linetype = "dashed", linewidth = 0.7) +
...
pdf(file.path(main_output_dir, "all-study-Asian.pdf"), width = 30, height = 10)
print(p1)
dev.off()
...
p <- ggplot(data1,aes(x=Author,y=Never_Smoker))+
  geom_point(size=6,position = position_dodge(width = 0.5), aes(colour = factor(Group)))+  
  theme_bw() + 
  facet_grid(. ~ Group, scales = "free_x", space = "free_x")+
  geom_hline(yintercept = 50, color = "black", linetype = "dashed", linewidth = 0.7) +
...
pdf(file.path(main_output_dir, "all-study-Never Smoker.pdf"), width = 30, height = 10)
print(p1)
dev.off()
