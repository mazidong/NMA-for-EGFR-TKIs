...
main_output_dir <- "/your_path_to_project/NMA-for-EGFR-TKI/outcomes/network-meta/" # Change to your main output directory
...
excel_path_default <- paste(file_path, "network_data.xlsx", sep = "")  # Original data containing the main figures 2, 3, and 4 and supplementary materials for network meta-analysis
excel_path_rare <- paste(file_path, "network_data_rare_events.xlsx", sep = "") # Original data for the sensitivity analysis of rare event in supplementary materials
...
supp_sheets <- c(
  ...
  "cardiac serious (dose)", "cardiac total (dose)", "vascular serious (dose)","vascular total (dose)",#The above are the data used for multi-dose group
...
color_list <- c("cardiac serious (0)" = "steelblue", 
                ...
                "cardiac serious (0)-gen"= "#8B0000", 
                "cardiac total (0)-gen"= "#8B0000", 
                "vascular serious (0)-gen"= "#8B0000", 
                "vascular total (0)-gen"= "#8B0000",
                ...
                "Serious Cardiac (G-E)-Gen" = "#8B0000",
                "Total Cardiac (G-E)-Gen" = "#8B0000",
                "Serious Vascular (G-E)-Gen" = "#8B0000",
                "Total Vascular (G-E)-Gen" = "#8B0000"
                ) 
...
run_network_analysis <- function(excel_path, sheet_names, input_settings, main_output_dir, method = "default") {
  for (sheet in sheet_names) {
    ...
    if (method == "default") {
      pw <- pairwise(treat = alloc1, n = sampleSize, event = responders,
                     studlab = study, data = df, sm = "OR", add = 0.5)
      net <- netmeta(pw, ref = "Placebo", common = FALSE)
    } else if (method == "inverse") {
      pw <- pairwise(treat = alloc1, n = sampleSize, event = responders,
                     studlab = study, data = df, sm = "OR")
      net <- netmetabin(pw, ref = "Placebo", method = "Inverse", details.chkmultiarm = TRUE)
    }
  ...
  skip_if_exists(funnel_file, function() {
    pdf(funnel_file)
    funnel(net, order=unique(net$treat1), pos.studlab = 4, legend = FALSE, linreg = TRUE, col.study = "black", digits.pval = 2, cex.studlab = 0.7,studlab=TRUE,level = 0.95, contour = c(0.9, 0.95, 0.99),col.contour = c("darkgrey", "grey", "lightgrey"),lwd = 2, cex = 1, pch = 16)
    dev.off()
  })
  f = funnel(net, order=unique(net$treat1), pos.studlab = 4, legend = FALSE, linreg = TRUE, col.study = "black", digits.pval = 2, cex.studlab = 0.7,studlab=TRUE,level = 0.95, contour = c(0.9, 0.95, 0.99),col.contour = c("darkgrey", "grey", "lightgrey"),lwd = 2, cex = 1, pch = 16)
  ...
  skip_if_exists(netsplit_file, function() {
    pdf(netsplit_file, width = 8, height = 30)
...
head(final_result)
write.csv(final_result, "final_results_network_meta_outcomes.csv")
...
file_path1 <- paste0(base_dir,"network-meta-analysis-total_matrix.xlsx")
...
# The path to the data folder
cinema_output_dir <- paste0(file_path, "CINeMA_report/") # Set as your path

wb <- createWorkbook()

setwd(cinema_output_dir)
...
for (file in list.files(cinema_output_dir, pattern = "\\.csv$", full.names = FALSE)) {
  ...
  data <- read.csv(file, stringsAsFactors = FALSE, header = TRUE, check.names = FALSE)
...
saveWorkbook(wb, "network-meta-analysis-CINEMA.xlsx", overwrite = TRUE)
...
cinema_path = paste0(cinema_output_dir, 'network-meta-analysis-CINEMA.xlsx')
matrix_path = paste0(main_output_dir,"network-meta-analysis-total_matrix.xlsx")
...
saveWorkbook(wb, "network-meta-analysis-total_matrix-cinema.xlsx", overwrite = TRUE)
...
getwd()
cinema_path = paste0(cinema_output_dir, 'network-meta-analysis-CINEMA.xlsx')
outcome_path = paste0(base_dir, 'final_results_network_meta_outcomes.csv')
...
write.csv(outcome_final, "final_results_network_meta_outcomes_with_all_confidence.csv", row.names = FALSE)

print(head(outcome_final))
