# file name: 9. Bayesian-network-meta-analysis.r
...
main_output_dir <- here("outcomes", "Bayesian")
if (!dir.exists(main_output_dir)) dir.create(main_output_dir)

# The path to the data folder
excel_path <- here("input", "Bayesian-network-meta-data.xlsx") 
...
for (category_name in names(categories)) {
  ...
  for (i in categories[[category_name]]) {
    ...
    tryCatch({
      ...
      pathway <- file.path(main_output_dir, category_name)
      if (!dir.exists(pathway)) dir.create(pathway, recursive = TRUE)
      ...
      # Save forest plot
      pdf_file_forest <- file.path(pathway, paste0("Gemtc-forest-", i, ".pdf"))
      ...
      pdf_file_density <- file.path(pathway, paste0("Trace_density-", i, ".pdf"))
      ...
      # Diagnostic diagram
      pdf_file_diagnosis <- file.path(pathway, paste0("Diagnosis-", i, ".pdf"))
      ...
      write.csv(sucra,file.path(pathway, paste0(i,"_gemtc_sucra.csv")))
      ...
      # Consistency test
      pdf_file_diagnosis1 <- file.path(pathway, paste0("Consistency-", i, ".pdf"))
      ...
      pdf_file_diagnosis2 <- file.path(pathway, paste0("Heterogeneity-", i, ".pdf"))
      skip_if_exists(pdf_file_diagnosis2 , function() {
      pdf(pdf_file_diagnosis2, width = 14)
      plot(c)
      dev.off()
      ...
      write.csv(a, file.path(pathway, paste0("Gmmtc-matrix_",i,".csv")))
...
primary_dir <- file.path(main_output_dir, "primary")
if (!dir.exists(primary_dir)) dir.create(primary_dir)

test <- data.frame(group = character())
list.files(path = primary_dir, pattern = "*\\gemtc_sucra.*\\.csv$")
for (file in list.files(path = primary_dir, pattern = "*\\gemtc_sucra.*\\.csv$", full.names = TRUE)){
      if (grepl("csv", file)){
            print(file)
...
#colnames(test)
write.csv(test, file.path(primary_dir, "primary outcomes-sucra-heatmap.csv"))

secondary_dir <- file.path(main_output_dir, "secondary")
if (!dir.exists(secondary_dir)) dir.create(secondary_dir)

test <- data.frame(group = character())
list.files(path =  secondary_dir, pattern = "*\\gemtc_sucra.*\\.csv$")
for (file in list.files(path =  secondary_dir, pattern = "*\\gemtc_sucra.*\\.csv$", full.names = TRUE)){
      if (grepl("csv", file)){
            print(file)
            data <- read.csv(file, header = TRUE)
            colnames(data) <- c('group', str_split(file,'_')[[1]][1])
...
colnames(test)
write.csv(test, file.path(secondary_dir, "secondary outcomes-sucra-heatmap.csv"))



primary <- read.csv(file.path(main_output_dir, 'primary', "primary outcomes-sucra-heatmap.csv"), check.names = F, row.names = 1)
secondary <- read.csv(file.path(main_output_dir, 'secondary', "secondary outcomes-sucra-heatmap.csv"), check.names = F, row.names = 1)
...
excel_path1 <- here("input", "primary-secondary-outcomes-sucra-heatmap.xlsx") 
...
pdf(file.path(main_output_dir, "SUCRA-heatmap-alltreatments.pdf"), height = 14, width = 10)
plot(all)
dev.off()

pdf(file.path(main_output_dir, "SUCRA-heatmap-interest-EGFR-TKI.pdf"), height = 8, width = 8)
plot(interest)
plot(EGFR_TKI)
dev.off()
