...
main_output_dir <- here("outcomes", "forest-plot")

if (!dir.exists(main_output_dir)) dir.create(main_output_dir)

# The path to the data folder
file <- here("input", "Figure5-forestplot-data.xlsx")# Used to analyze primary results
file1 <-  here("input", "Figure5-forestplot-data-appendix.xlsx")# Used to analyze secondary results
...
primary_path <- file.path(main_output_dir, "primary-serious-total-forestplot.pdf")
pdf(primary_path, width = 12)
...
secondary_path <- file.path(main_output_dir, "secondary-serious-total-forestplot.pdf")
pdf(secondary_path, width = 12)
...
write.csv(csm_vsm_data, file.path(main_output_dir, 'cardiac-vascular-serious-meta.csv'))
write.csv(ctm_vtm_data, file.path(main_output_dir, 'cardiac-vascular-total-meta.csv'))
...
write.csv(arsm_hfsm_vtsm_hysm_data, file.path(main_output_dir, 'arrhythmias-heart failure-Vascular Toxicity-hypertension-serious-meta.csv'))
write.csv(artm_hftm_vttm_hytm_data, file.path(main_output_dir, 'arrhythmias-heart failure-Vascular Toxicity-hypertension-total-meta.csv'))
...
# The path to the data folder
data <- read.csv(here("input","primary-serious-total-meta.csv"), header = TRUE, check.names = F)
...
plot(all_p1)
pdf(file.path(main_output_dir, "Figure5-forestplot.pdf"), width = 15, height = 6)
plot(all_p1)
dev.off()
...
data <- read.csv(here("input", "secondary-serious-total-meta.csv"), header = TRUE, check.names = F)
...
pdf(file.path(main_output_dir, "Figure5-forestplot-appendix.pdf"), width = 15, height = 8)
plot(all_p1)
dev.off()
